...
3. **嵌入式终端** - 内置 SSH 终端，支持与远程设备进行交互式命令行操作
4. **代码生成** - 根据产品型号生成初始化模板、驱动代码（规划中）
5. **教程文档** - 配置指南、代码示例与版本说明
...
# 安装依赖
conda run -n pyqt5_env pip install PyQt5 PyQt-Fluent-Widgets paramiko pyte wcwidth
```
...
#### 异步 SSH 任务
为了避免 UI 冻结，耗时操作（如 SSH 连接、命令执行）应在子线程中运行：
```python
# 使用 QThread 和 QObject
self.thread = QThread()
self.worker = SshConnectWorker(host, user, pwd)
...
1. **HomeInterface** (`home_interface.py`)
   - 主页展示，采用卡片式布局快速进入功能模块。
2. **TerminalInterface** (`terminal_interface.py`)
   - 基于 `paramiko` 和 `pyte` 实现的终端模拟器。
3. **ConfigManager** (`config_manager.py`)
   - 统一管理 `settings.json` 的读写，支持默认值合并。
4. **FontManager** (`font_manager.py`)
   - 响应 `settings.json` 中的字体大小设置，动态调整全局 QSS 和控件字体。
5. **StyleHelper** (`style_helper.py`)
   - 提供辅助函数生成符合当前字体设置的 CSS 样式字符串。
